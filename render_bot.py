...
SYSTEM_PROMPT = """Ты — редактор Telegram-канала «Кот в матрице». Тема: коты + IT/технологии.

ПРАВИЛА (строго соблюдай):
1. Ответь ТОЛЬКО текстом поста. Никаких предисловий, рассуждений, подсчётов.
2. Объём: ровно 50–200 символов. Коротко, смешно, понятно.
3. Начни с эмодзи: 🐱 😼 😺 😸 🐾 🤖 ⚡ 🧠 💻 🔧 🖥️ 📱 💾
4. Пиши на русском, без хэштегов, без ссылок.
5. Только один пост — без вариантов, без пояснений.
...
CAT_FACTS = [
    ...
    "Ctrl+Z не работает, когда кот прошёлся по клавиатуре.",
    ...
    "Кот-продакт: требует фичу «корм в 3 утра» уже пятый год.",
    ...
    "Кот не делает ошибки. Он делает экспериментальные фичи.",
    "Системные требования кота: 1 миска, 2 уха, 0 багов.",
    "Кот не мурлычет — он оптимизирует звуковой движок.",
    "Если кот трогает лапкой воду — он тестирует API.",
    "Кот не мяукает. Он отправляет запрос на кормление.",
    "Каждый кот — это микросервис с урчанием.",
    "Кот не дерёт диван. Он проводит стресс-тест обивки.",
    "Кот на клавиатуре — не помеха, а коллега.",
    "В отличии от кода, кот всегда проходит код-ревью.",
    "Кот не роняет вещи. Он перераспределяет память.",
    "Sleep(кот) = while(True) мурлыкать.",
]
